      ...
      >
        ...
        <div className={`flex flex-col justify-between p-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50 ${hasImage ? "w-full sm:w-3/5" : "w-full"} min-h-[80px] sm:min-h-0`}>
          <div className="flex-1 space-y-1">
            <p className="line-clamp-2 text-sm font-medium text-gray-900 dark:text-gray-100 leading-tight transition-colors">{displayTitle}</p>
            {previewDesp && <p className="line-clamp-2 sm:line-clamp-3 text-xs leading-relaxed text-gray-600 dark:text-gray-400 transition-colors">{previewDesp}</p>}
            {hasError && <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-1.5 py-0.5 rounded transition-colors">Preview unavailable</p>}
          </div>
          <div className="mt-2 pt-1 border-t border-gray-100 dark:border-gray-700 flex-shrink-0 transition-colors">
            <div className="flex items-baseline space-x-1.5">
              ...
              />
              <svg className="w-3 h-3 flex-shrink-0 text-gray-400 dark:text-gray-500 hidden transition-colors" fill="currentColor" viewBox="0 0 20 20" style={{ verticalAlign: "baseline" }}>
                <path
                  fillRule="evenodd"
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 font-mono break-all transition-colors">{hostname}</span>
            </div>
          </div>
        </div>
        ...
        {hasImage && (
          <div className="relative w-full h-24 sm:w-2/5 sm:h-full flex-shrink-0 bg-gray-100 dark:bg-gray-800 transition-colors">
            <OptimizedImage
              src={previewImage}
              alt={displayTitle}
              fill
      ...
      <div className={`flex flex-col sm:flex-row overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 h-auto sm:h-24 md:h-28 transition-colors ${outfit.className}`}>
        ...
        <div className="flex w-full sm:w-3/5 animate-pulse flex-col justify-between p-3 min-h-[80px] sm:min-h-0">
          <div className="flex-1 space-y-2">
            <div className="h-4 w-3/4 rounded-md bg-gray-200 dark:bg-gray-700 transition-colors"></div>
            <div className="space-y-1">
              <div className="h-3 w-full rounded-md bg-gray-200 dark:bg-gray-700 transition-colors"></div>
              <div className="h-3 w-4/5 rounded-md bg-gray-200 dark:bg-gray-700 transition-colors"></div>
            </div>
          </div>
          <div className="mt-2 pt-1 border-t border-gray-100 dark:border-gray-700 flex-shrink-0 transition-colors">
            <div className="flex items-center space-x-1.5">
              <div className="w-3 h-3 rounded-sm bg-gray-200 dark:bg-gray-700 transition-colors"></div>
              <div className="h-3 w-1/2 rounded-md bg-gray-200 dark:bg-gray-700 transition-colors"></div>
            </div>
          </div>
        </div>

        {/* Image skeleton */}
        <div className="w-full h-24 sm:w-2/5 sm:h-full bg-gray-100 dark:bg-gray-800 transition-colors">
          <div className="size-full bg-gray-200 dark:bg-gray-700 animate-pulse transition-colors"></div>
        </div>
      </div>
    </div>
  )
